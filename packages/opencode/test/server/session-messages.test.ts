...
describe("session messages endpoint", () => {
  ...
  it.instance(
    "emits followable next links for after pages",
    withoutWatcher(
      Effect.gen(function* () {
        const session = yield* sessionScoped
        yield* fill(session.id, 5, (i: number) => 1_000 + i)
        ...
        expect(newest.headers["link"]).toContain("after=")

        const match = /<([^>]+)>/.exec(newest.headers["link"] ?? "")
        expect(match?.[1]).toBeTruthy()
        const older = yield* request(new URL(match![1]!).pathname + new URL(match![1]!).search)
